#!/usr/bin/env node
/**
 * yourcms-cli — zero-dependency CLI for the CMS management API.
 *
 *   yourcms-cli login --host http://localhost:8000 --email you@x.com --password ...
 *   yourcms-cli spaces
 *   yourcms-cli types export --space <spaceId> --env master -o types.json
 *   yourcms-cli types import --space <spaceId> --env master -i types.json
 *   yourcms-cli generate-types --space <spaceId> --env master -o cms-types.d.ts
 *
 * Credentials live in ~/.yourcmsrc.json (host + access/refresh tokens); the
...
const RC_PATH = join(homedir(), '.yourcmsrc.json');
...
async function api(rc, path, options = {}, retryOn401 = true) {
  ...
  if (res.status === 401 && retryOn401 && rc.refresh_token) {
    ...
    if (!refreshed.ok) fail('Session expired — run `yourcms-cli login` again.');
...
async function cmdGenerateTypes(rc, flags) {
  ...
  let out = `/* Generated by yourcms-cli from space ${space} (${env}) — do not edit.\n`;
...
if (needsAuth && !rc.access_token) fail('Not logged in — run: yourcms-cli login --host <url> --email <email> --password <pw>');
...
switch (command) {
  ...
  default:
    console.log(`yourcms-cli — commands:
