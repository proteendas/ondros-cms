...
| Superadmin app | `superadmin/` (Next.js) | Vercel (second project, optional) | Same. |
...
deployed as its own Vercel project — see that repo's own README for setup.
It makes no backend calls, so it isn't part of this stack's `CORS_ORIGINS`.
...
This is a monorepo — every app (`backend/`, `editor/`, `preview/`, …) has
...
In the **`ondros-cms-site`** repo's own Vercel project, set:
...
These are inlined into the client bundle at build time — redeploy that
project whenever the editor's URL changes.
...
1. Open the marketing site (its own Vercel deployment) → click **Login** →
   lands on the editor login.
...
The marketing site (`ondros-cms-site`) runs separately — clone that repo,
`npm install && npm run dev`, and point its `NEXT_PUBLIC_APP_LOGIN_URL` /
`NEXT_PUBLIC_APP_SIGNUP_URL` at `http://localhost:3000/login` and `/signup`.
